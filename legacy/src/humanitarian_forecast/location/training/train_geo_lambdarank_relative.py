#!/usr/bin/env python3
"""Train an Ethiopia-specialized LambdaRank with explicit within-query relative features.
...
RELATIVE_INDICES = np.asarray([
    4,5,9,11,12,13,14,15,16,17,18,23,24,25,26,27,
    52,53,54,55,56,57,58,59,
], dtype=np.int64)


def _relative_candidate_features(candidates: np.ndarray, valid: np.ndarray) -> np.ndarray:
    """Per-query z-scores and percentile ranks for high-value candidate signals."""
    selected = candidates[..., RELATIVE_INDICES]
    masked = np.where(valid[..., None], selected, np.nan)
    mean = np.nanmean(masked, axis=1, keepdims=True)
    std = np.nanstd(masked, axis=1, keepdims=True)
    z = np.nan_to_num((selected - mean) / np.maximum(std, 1e-5), nan=0.0, posinf=0.0, neginf=0.0)
    z = np.clip(z, -8.0, 8.0)
    sortable = np.where(valid[..., None], selected, np.inf)
    order = np.argsort(sortable, axis=1, kind="stable")
    ranks = np.empty_like(order, dtype=np.float32)
    ordinal = np.broadcast_to(np.arange(candidates.shape[1], dtype=np.float32)[None, :, None], order.shape)
    np.put_along_axis(ranks, order, ordinal, axis=1)
    denom = np.maximum(valid.sum(axis=1, keepdims=True) - 1, 1).astype(np.float32)
    percentile = ranks / denom[..., None]
    percentile = np.where(valid[..., None], percentile, 0.0)
    return np.concatenate([z.astype(np.float32), percentile.astype(np.float32)], axis=-1)
...
def _flatten_features(
    ...
    relative = _relative_candidate_features(candidates, valid)
    full = np.concatenate([candidates, geometry, recent_distance, relative], axis=-1).astype(np.float32, copy=False)
...
def main() -> None:
    ...
    parser.add_argument("--ethiopia-weight", type=float, default=12.0)
    parser.add_argument("--regional-weight", type=float, default=2.0)
    ...
    train_countries = [str(meta[i].get("country", "")).casefold() for i in range(train_end)]
    regional = {"ethiopia", "eritrea", "somalia", "sudan", "south sudan", "djibouti", "kenya"}
    query_weight = np.ones(train_end, dtype=np.float32)
    for i, country_name in enumerate(train_countries):
        if country_name == "ethiopia":
            query_weight[i] = args.ethiopia_weight
        elif country_name in regional:
            query_weight[i] = args.regional_weight
    row_weight = np.repeat(query_weight, train_group).astype(np.float32, copy=False)
    print(
        f"train_rows={len(train_x):,} validation_rows={len(val_x):,} features={train_x.shape[1]} "
        f"ethiopia_queries={(np.asarray(train_countries)=="ethiopia").sum()} "
        f"ethiopia_weight={args.ethiopia_weight} regional_weight={args.regional_weight}",
        flush=True,
    )
    ...
    best: tuple[float, int, float, dict[str, float], dict[str, float]] | None = None
    ...
    for iteration in steps:
        ...
        eth_mask = np.asarray([str(meta[i].get("country", "")).casefold() == "ethiopia" for i in range(train_end, valid_end)])
        eth_report = metrics(
            raw[eth_mask],
            _softmax(raw[eth_mask], validation_valid[eth_mask], temperature),
            coordinates[train_end:valid_end][eth_mask],
            validation_valid[eth_mask],
            target[train_end:valid_end][eth_mask],
        )
        score = eth_report["broad_area_score"]
        print(json.dumps({"iteration": iteration, "temperature": temperature, "validation_global": report, "validation_ethiopia": eth_report}), flush=True)
        if best is None or score > best[0]:
            best = (score, iteration, temperature, report, eth_report)
    assert best is not None
    _, best_iteration, temperature, validation_report, validation_ethiopia = best
    ...
    dev_eth_mask = np.asarray([str(meta[i].get("country", "")).casefold() == "ethiopia" for i in range(valid_end, n)])
    development_ethiopia = metrics(
        dev_raw[dev_eth_mask],
        dev_probability[dev_eth_mask],
        coordinates[valid_end:][dev_eth_mask],
        valid[valid_end:][dev_eth_mask],
        target[valid_end:][dev_eth_mask],
    )
    ...
    config = {
        ...
        "feature_dim": int(candidates.shape[-1] + 12 + 4 + 2*len(RELATIVE_INDICES) + 68),
    ...
    report = {
        ...
        "validation_ethiopia": validation_ethiopia,
        "development": development_report,
        "development_ethiopia": development_ethiopia,
        "configuration": config,
        "protocol": "70% global replay train with Ethiopia/regional query weighting; 15% Ethiopia validation selects tree count; final 15% development benchmark",
    ...
    write_info(
        args.output_dir,
        ModelInfo(
            subsystem="location/geo_lambdarank_relative",
            version=args.version,
            status="research-challenger",
            description="Ethiopia-specialized LambdaRank with global replay and explicit within-query candidate z-scores/percentile ranks.",
            metrics={"validation": validation_report, "validation_ethiopia": validation_ethiopia, "development": development_report, "development_ethiopia": development_ethiopia},
            lineage={
            ...
            training={
                ...
                "num_leaves": args.num_leaves,
                "seed": args.seed,
                "ethiopia_weight": args.ethiopia_weight,
                "regional_weight": args.regional_weight,
            },
            calibration={"temperature": temperature, "selection": "validation broad_area_score"},
            notes=[
                "Promoted v9 remains untouched.",
                "Adds explicit within-query ranks/z-scores for 24 recurrence, transition, raw-event and actor-transfer signals.",
            ],
        ),
    )
