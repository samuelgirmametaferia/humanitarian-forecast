...
def _build_regression_rows(
    ...
    for start in range(lo, hi, chunk):
        ...
        move_label = (np.linalg.norm(residual, axis=1) * SCALE_KM > 25.0).astype(np.int32)
        ...
        xs.append(f); ys.append(residual); ws.append(weight); gates.append(move_label)
        print(f"refiner rows {stop:,}/{hi:,} accumulated={sum(len(v) for v in xs):,}", flush=True)
    return np.concatenate(xs), np.concatenate(ys), np.concatenate(ws), np.concatenate(gates)
...
def _predict_matrices(
    ...
    move_flat = gate_model.predict_proba(f)[:, 1]
    ...
    move = np.zeros(valid.shape, dtype=np.float32)
    move[valid] = move_flat.astype(np.float32)
    return raw, residual, move
...
def main() -> None:
    ...
    train_x, train_y, train_w, train_gate = _build_regression_rows(
    ...
    gate = LGBMClassifier(
        objective="binary", n_estimators=max(250, a.estimators//2), learning_rate=.04,
        num_leaves=47, min_child_samples=120, colsample_bytree=.80,
        reg_lambda=6.0, reg_alpha=.3, class_weight="balanced", n_jobs=-1,
        verbosity=-1, random_state=a.seed+2,
    )
    gate.fit(train_x, train_gate, sample_weight=train_w)
    del train_x, train_y, train_w, train_gate
    ...
    val_scores, val_res, val_move = _predict_matrices(
        east, north, gate, coarse, x[tr:va], candidates[tr:va], coords[tr:va], valid[tr:va], target[tr:va], iteration
    )
    ...
    for cap in (25., 50., 75., 100.):
        ...
        for gamma in (0.0, 0.5, 1.0, 2.0, 4.0):
            gate_weight = np.ones_like(val_move) if gamma == 0.0 else np.power(np.clip(val_move, 1e-4, 1.0), gamma)
            for alpha in np.linspace(0., 1.25, 6):
                refined = coords[tr:va] + float(alpha)*gate_weight[...,None]*capped
                ...
                row={"cap_km":cap,"alpha":float(alpha),"gate_gamma":float(gamma),"global":global_report,"ethiopia":local_report}
                search.append(row)
    ...
    dev_scores, dev_res, dev_move = _predict_matrices(
        east, north, gate, coarse, x[va:], candidates[va:], coords[va:], valid[va:], target[va:], iteration
    )
    dev_prob = _softmax(dev_scores, valid[va:], temperature)
    dev_gate = np.ones_like(dev_move) if best["gate_gamma"] == 0.0 else np.power(np.clip(dev_move, 1e-4, 1.0), best["gate_gamma"])
    dev_refined = coords[va:] + best["alpha"]*dev_gate[...,None]*_cap_residual(dev_res, best["cap_km"])
    ...
    gate.booster_.save_model(str(a.output_dir/"move_gate.txt"))
    report={
        "schema":"candidate-continuous-refiner-gated-v2",
        "selected":{"alpha":best["alpha"],"cap_km":best["cap_km"],"gate_gamma":best["gate_gamma"]},
    ...
    write_info(a.output_dir,ModelInfo(
        subsystem="location/candidate_refiner",version="v2",status="research-challenger",
        description="Gated continuous coarse-to-fine residual refinement of frozen actor-transfer LambdaRank candidate mass.",
        ...
        training={"estimators":a.estimators,"k_near":a.k_near,"training_radius_km":a.training_radius_km,"selected_alpha":best["alpha"],"selected_cap_km":best["cap_km"],"gate_gamma":best["gate_gamma"],"seed":a.seed},
        notes=["The coarse ranking/order and probabilities are frozen; only candidate coordinates are locally refined.","A training-only classifier estimates whether a candidate is already within 25 km and suppresses unnecessary movement.","alpha=0 is included as an explicit validation fallback.","Development is diagnostic, not pristine prospective evaluation."],
    ))
