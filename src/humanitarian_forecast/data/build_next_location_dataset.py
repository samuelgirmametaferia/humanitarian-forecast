...
def event_features(
    ...
    rw: dict[tuple[str, str], np.ndarray], include_motion: bool,
) -> np.ndarray:
...
def main() -> None:
    ...
    parser.add_argument(
        "--feature-set", choices=("geo-v2", "motion-v3"), default="motion-v3",
        help="geo-v2 reproduces the promoted 19-feature v9 history input; motion-v3 adds six movement features.",
    )
    ...
    for conflict_id, events in conflicts.items():
        for target_index in range(args.minimum_history, len(events)):
            ...
            anchor = history[-1]
            feature_dim = 19 if args.feature_set == "geo-v2" else 25
            tensor = np.zeros((args.sequence_length, feature_dim), dtype=np.float32)
            encoded = [
                event_features(
                    event, history[max(0, index - 1)], anchor, target["date"], rw,
                    include_motion=args.feature_set == "motion-v3",
                )
    ...
    print(f"conflicts={len(conflicts):,} samples={len(xs):,} shape={(len(xs), args.sequence_length, xs[0].shape[-1])} feature_set={args.feature_set}")
