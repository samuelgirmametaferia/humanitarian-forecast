...
export function MethodologyRoute() {
  return <ContentRoute title="Methodology">
    ...
    <section><h2>Updates</h2><p>New observations enter later training runs automatically: every published forecast's inputs are stored, labeled against verified outcomes once the horizon has matured, and a scheduled weekly retrain reuses them. A retrained model is promoted only after parity validation against its source checkpoint, and the serving layer adopts the promoted version from the model registry on its next forecast. An update appears in History when a run actually occurs.</p></section>
