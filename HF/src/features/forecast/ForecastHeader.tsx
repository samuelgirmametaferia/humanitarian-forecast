...
export function ForecastHeader({ data }: { data: ForecastSnapshot }) {
  ...
  return <header className="forecast-header">
    ...
    <div className="forecast-meta"><div><span>Last update</span><time dateTime={data.generatedAt}>{formatDate(data.generatedAt, true)}</time></div><ModelPicker /><button type="button" onClick={() => setTable(true)}><Table2 aria-hidden="true" />View data</button></div>
  </header>
}
