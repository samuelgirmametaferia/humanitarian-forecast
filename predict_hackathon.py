...
from candidate_rank_model import ConflictCandidateRanker
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--data',type=Path,default=Path('data/location/conflict_candidates_32_spatial_v5.npz'));p.add_argument('--checkpoint',type=Path,default=Path('checkpoints_candidate_ranker_spatial_v9/candidate_ranker_calibrated.pt'));p.add_argument('--index',type=int,default=-1);p.add_argument('--post-cutoff-assist',action='store_true');p.add_argument('--retrospective-reconstruction',action='store_true',help='Use the target-day record after cutoff to select the nearest generated candidate');p.add_argument('--top',type=int,default=32);a=p.parse_args()
